...
// Sets confirmed=true for the matching subscriber. Returns an HTML confirmation page.
...
export default async function handler(req, res) {
  ...
  if (!token) {
    ...
    return res.status(400).send(
      page("Invalid link", "<p>No confirmation token found. Try clicking the link in your email again.</p>")
    );
  }
  ...
  if (error || !data || data.length === 0) {
    ...
    return res.status(404).send(
      page("Link not found", "<p>This confirmation link is not recognised. You may have already confirmed, or the link may be invalid.</p><p><a href='/'>Back to Rumbo</a></p>")
    );
  }

  res.setHeader("Content-Type", "text/html");
  return res.status(200).send(
    page("Subscription confirmed", "<p>You're in. Your first edition arrives on the next day you selected.</p><p><a href='/'>Read today's edition</a></p>")
  );
}
